...
public class MovieSearchSteps {
	...
	public void addNewMovie(String title, String producer, String releaseDate) throws ParseException {
		Movie movie = new Movie(title, producer, releaseDate);
		catalog.addMovie(movie);
	}
	
	@When("the following movies are added:")
	public void addNewMovies(List<Movie> listOfMovies)  {
		catalog.addMovies(listOfMovies);
	}
	...
	public void addAnotherMovie() throws ParseException {
		Movie movie = new Movie("Out of Africa", "Universal Pictures",
				new SimpleDateFormat( "dd MMMMM yyyy").parse("20 December 1985"));
		catalog.addMovie(movie );
	}
	...
	public void addYetAnotherMovie() throws ParseException {
		Movie movie = new Movie("Avatar", "20th Century Fox",
				new SimpleDateFormat("dd MMMMM yyyy" ).parse("18 December 2009"));
		catalog.addMovie(movie );
	}*/
	...
	public void initializeEmptyCatalog (){
		catalog.clearMovies();
	}
	
	
	@When("a movie with a title '(.+)' and a producer '(.+)' with the release date of (.+) is added")
	public void addAnotherMovie(String title, String producer, String releaseDate) throws ParseException {
		Movie movie = new Movie(title, producer, releaseDate);
		catalog.addMovie(movie);
	}
	
	@When("no movies are added")
	public void noChangeInCatalog () {
		assertThat(catalog.getSize (), equalTo(0));
	}
	
	@Then("the catalog has (\\d+) movies?" )
	public void confirmCatalogSize(int quantity) { 
		assertThat(catalog.getSize (), equalTo(quantity));
	}
	
	@Then("the catalog should have 3 total movies" )
	public void verifyCatalogSize() { 
